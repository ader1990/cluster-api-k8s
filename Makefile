...
generate-bootstrap-conversions: $(CONVERSION_GEN)
	$(CONVERSION_GEN) \
		--input-dirs=./bootstrap/api/v1beta2 \
		--extra-peer-dirs=sigs.k8s.io/cluster-api/api/v1beta2 \
...
generate-controlplane-conversions: $(CONVERSION_GEN)
	$(CONVERSION_GEN) \
		--input-dirs=./controlplane/api/v1beta2 \
		--extra-peer-dirs=sigs.k8s.io/cluster-api/api/v1beta2 \
		--extra-peer-dirs=github.com/canonical/cluster-api-k8s/bootstrap/api/v1beta2 \
		--build-tag=ignore_autogenerated_conversions \
		--output-file-base=zz_generated.conversion \
		--output-base=./ \
		--go-header-file=./hack/boilerplate.go.txt
