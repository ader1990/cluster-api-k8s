...
type DrainOptions struct {
	// Timeout is the maximum duration to wait for the a single cycle of drain operation to complete.
	...
	AllowDeletion bool
	// EvictionRetryInterval is the duration to wait between retries when evicting pods.
...
func (d *drainer) DrainNode(ctx context.Context, nodeName string) error {
	...
	for {
		select {
		case <-ctx.Done():
			return fmt.Errorf("drain operation timed out or cancelled: %w", ctx.Err())
		case <-ticker.C:
