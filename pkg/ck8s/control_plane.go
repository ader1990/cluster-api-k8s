...
func (c *ControlPlane) FailureDomainWithMostMachines(ctx context.Context, machines collections.Machines) string {
	controlPlaneFDs := filterControlPlaneFailureDomains(c.FailureDomains())
	controlPlaneFDIDs := failureDomainIDs(controlPlaneFDs)

	// See if there are any Machines that are not in currently defined failure domains first.
	notInFailureDomains := machines.Filter(
		collections.Not(collections.InFailureDomains(controlPlaneFDIDs...)),
	)
	...
	return failuredomains.PickMost(ctx, controlPlaneFDs, c.Machines, machines)
}

// NextFailureDomainForScaleUp returns the failure domain with the fewest number of up-to-date machines.
func (c *ControlPlane) NextFailureDomainForScaleUp(ctx context.Context) string {
	controlPlaneFDs := filterControlPlaneFailureDomains(c.FailureDomains())
	if len(controlPlaneFDs) == 0 {
		return ""
	}
	return failuredomains.PickFewest(ctx, controlPlaneFDs, c.Machines, c.UpToDateMachines())
}
...
func (c *ControlPlane) NewMachine(infraRef, bootstrapRef *corev1.ObjectReference, failureDomain string) *clusterv1.Machine {
	return &clusterv1.Machine{
		...
		Spec: clusterv1.MachineSpec{
			...
			Version:           c.Version(),
			InfrastructureRef: clusterv1.ContractVersionedObjectReference{Name: infraRef.Name, Kind: infraRef.Kind, APIVersion: infraRef.APIVersion, Namespace: infraRef.Namespace},
			Bootstrap: clusterv1.Bootstrap{
				ConfigRef: clusterv1.ContractVersionedObjectReference{Name: bootstrapRef.Name, Kind: bootstrapRef.Kind, APIVersion: bootstrapRef.APIVersion, Namespace: bootstrapRef.Namespace},
			},
...
func (c *ControlPlane) MachinesNeedingRollout() collections.Machines {
	...
	return machines.Filter(collections.Not(machinefilters.MatchesKCPConfiguration(c.infraResources, c.ck8sConfigs, c.KCP)))
}
...
func getInfraResources(ctx context.Context, cl client.Client, machines collections.Machines) (map[string]*unstructured.Unstructured, error) {
	...
	for _, m := range machines {
		infraObj, err := external.Get(ctx, cl, &corev1.ObjectReference{
...
func getCK8sConfigs(ctx context.Context, cl client.Client, machines collections.Machines) (map[string]*bootstrapv1.CK8sConfig, error) {
	...
	for _, m := range machines {
		...
		if bootstrapRef == (clusterv1.ContractVersionedObjectReference{}) {
			continue
		}
...
func filterControlPlaneFailureDomains(fds clusterv1.FailureDomains) []clusterv1.FailureDomain {
	if len(fds) == 0 {
		return nil
	}

	filtered := make([]clusterv1.FailureDomain, 0, len(fds))
	for _, fd := range fds {
		if fd.ControlPlane {
...
func failureDomainIDs(fds []clusterv1.FailureDomain) []string {
	if len(fds) == 0 {
		return nil
	}
